...
function Usuarios() {
    ...
    },
    {
      ...
      render: (usuario: Usuario) => (
        <div className="flex justify-end gap-2">
          <Link href={`/usuarios/${usuario.id}/editar`} title="Editar Usuário">
          ...
          <Button
            ...
            className={`p-2 ${usuario.status === 'ATIVO' ? 'hover:text-red-600 hover:bg-red-50' : 'hover:text-emerald-600 hover:bg-emerald-50'}`}
  ...
  return (
    <div className="p-8 bg-slate-50 min-h-screen">
      ...
      <DataTable
        data={usuarios.filter(u => u.status !== 'DELETADO').sort((a, b) => (a.id ?? 0) - (b.id ?? 0))}
        columns={columns}
        keyExtractor={(u) => u.id ?? 0}
        isLoading={isLoading}
